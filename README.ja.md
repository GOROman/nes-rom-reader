...
📖 **[組み立てガイド(初心者向け・ステップバイステップ)](docs/assembly.md)** — 必要な工具、はんだ付けの順番、チェック方法、トラブル対処。

> ⚠️ **v0.1 の既知の問題**: M5Stamp S3 がそのまま挿さりません(Stamp装着部のフットプリントが実物と不一致)。
> 回避策はピンヘッダでの嵩上げ実装(組み立てガイドに記載)。v0.2 で修正予定です。

## 対応カセットと制限

- ✅ **mapper 0 (NROM)** — 完全対応(PRG 32KB + CHR 8KB)、実機確認済み
- 🧪 **CNROM (mapper 3)** — **バスコンフリクト方式**でCHRバンク切替に対応(実験的)
- ⚠️ **UNROM / MMC1 など** — 未対応

**CNROMがv0.1基板でも動く理由**: 本基板のデータバッファ(74LVC245)は方向固定の読み出し専用で
データバスを駆動できません。しかし CNROM では**書き込みサイクル中もPRG-ROMがバスを駆動し続ける**
(AND型の[バスコンフリクト](https://www.nesdev.org/wiki/Bus_conflict))ため、
ゲーム自身も「書きたい値と同じ値が入っているROMアドレスへ書く」方法を使っています。
本ツールも同様に、先にPRGを吸い出し→目的のバンク番号と同じ値を持つアドレスを探し→
そこへダミーライトサイクルを発生させることで、カートリッジ側のラッチに値を取り込ませます。

CLIは `--bank-switch`、Web UIは **CHRバンク切替** のチェックで有効になります。

> ⚠️ **コピープロテクト用ダイオード**を持つCNROM基板は、意図的な追加バスコンフリクトがあり、
> バスを実際に駆動できるダンパーでないと勝てません。これらは書き込み対応の v0.2 基板が必要です。

**MMC1 / MMC3 系マッパーが吸い出せない理由**: これらのマッパーはバスコンフリクトを
起こしません(書き込みサイクル中にPRG-ROMがバスを駆動しない)。つまり上記の手法では
ラッチさせる値がバス上に存在せず、バンク選択には本当にデータバスを駆動する必要があります。
本基板は読み出し専用なのでそれができません。電源投入時にマップされている32KBのPRG窓と、
バッテリバックアップRAM(上記)は読めますが、ROM全体は吸い出せません。MMC3派生の
非ライセンスマッパー、たとえば **mapper 165**(*Fire Emblem (Unl)*。MMC2/MMC3ハイブリッドで
MMC2式CHRラッチと4KB CHR-RAMを持つ)も同様です。完全対応にはデータバッファを
カートリッジ側へ駆動できる基板リビジョンが必要です。
...
セーブ用電池を積んだカセット(ファイアーエムブレム、ゼルダ、ウィザードリィ等)は
`$6000-$7FFF` のWRAMにセーブデータを保持しています。Web UI の **セーブデータ** カード
(CLIなら `--save-out game.sav`)で読み出し、`.sav` ファイルとして保存できます。

動作するのは、電源投入時にWRAMが有効になっているマッパーのカセットです。本基板は
読み出し専用でデータバスを駆動できないため、WRAMを有効化するマッパーレジスタ
(MMC1はPRGレジスタのbit4、MMC3は `$A001`)を書けません。そのビットがOFFで起動する
カセットでは全バイト同一の値が返り、その旨を警告します。カセットへセーブデータを
書き戻す機能はありません。
...
| `W <addr_hex> <len_hex>` | バッテリバックアップRAM($6000-$7FFF)読み出し。応答形式は `R`/`C` と同じ |
