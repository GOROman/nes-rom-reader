...
- ✅ **mapper 0 (NROM)** … 完全対応(PRG 32KB + CHR 8KB)。実機で吸い出し確認済み
- ⚠️ **バンク切替式マッパー(CNROM / UNROM / MMC1 等)は未対応**

**理由(重要)**: バンク切替は「マッパーレジスタへの**書き込み**」が必要ですが、本基板のデータバッファ
(74LVC245)は**方向固定でカートリッジ→ESP32の読み出し専用**です。書き込みができないため、
CHR/PRGバンクを切り替えられません。

例: **トランスフォーマー コンボイの謎**(CNROM / mapper 3)は、PRG 32KBは吸えますが、
CHRは電源投入時の1バンク(8KB)しか読めません(残りのCHRバンクは切替に書き込みが必要)。

→ CNROM/UNROM/MMC1 等に対応するには、**データバッファを双方向化(書き込み対応)した v0.2 基板**と、
バンク切替を行うファームウェアが必要です。
